...
def calculate_molecule_3D_structure():
    def get_smiles_list_():
        data_df = pd.read_csv("/data/cjm/inhibitor_pre/data_case/raw/inhibitor_rdkit.csv")
    ...
    def calculate_molecule_3D_structure_(smiles_list):
        ...
        while True:
            ...
            if result != 0:
                ...
                mutex.acquire()
                with open('/data/cjm/inhibitor_pre/data_case/result/invalid_smiles.txt', 'a') as f:
    ...
    mutex = Lock()
    os.system('rm /data/cjm/inhibitor_pre/data_case/result/invalid_smiles.txt')
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_conformation_dict, open('/data/cjm/inhibitor_pre/data_case/intermediate/smiles_to_conformation_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_conformation_dict))


def construct_data_list():
    data_df = pd.read_csv("/data/cjm/inhibitor_pre/data_case/raw/inhibitor_rdkit.csv")
    smiles_to_conformation_dict = pkl.load(open('/data/cjm/inhibitor_pre/data_case/intermediate/smiles_to_conformation_dict.pkl', 'rb'))
    data_list = []
    ...
    pkl.dump(data_list, open('/data/cjm/inhibitor_pre/data_case/intermediate/data_list.pkl', 'wb'))


def convert_data_list_to_data_loader():
    def convert_data_list_to_dataset_(data_list):
        dictionary = Dictionary.load('/data/cjm/inhibitor_pre/data_case/raw/token_list.txt')
    ...
    batch_size = 6  # 原来为8
    data_list = pkl.load(open('/data/cjm/inhibitor_pre/data_case/intermediate/data_list.pkl', 'rb'))
    ...
    return data_loader_test  # 只返回测试集的DataLoader


class UniMolModel(nn.Module):
    def __init__(self):
        super().__init__()
        dictionary = Dictionary.load('/data/cjm/inhibitor_pre/test_case/raw/token_list.txt')
        dictionary.add_symbol("[MASK]", is_special=True)
...
def evaluate(model, data_loader, csv_save):
    ...
    if csv_save == True:
        ...
        df.to_csv('label_predict_case_sub15.csv', index=False)
...
if __name__ == "__main__":
    ...
    # print("train start!")
    # train(trial_version='inhi_seq_pocket_sub6')  # 表示训练集为全部训练集时的结果

    test(trial_version='inhi_seq_pocket_sub15')
    print("test start!")
