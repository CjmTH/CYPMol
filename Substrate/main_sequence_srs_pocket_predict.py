...
def calculate_molecule_3D_structure():
    def get_smiles_list_():
        data_df = pd.read_csv("/data/cjm/substrate/data_case/raw/substrate_rdkit.csv")
    ...
    def calculate_molecule_3D_structure_(smiles_list):
        ...
        while True:
            ...
            if result != 0:
                ...
                mutex.acquire()
                with open('/data/cjm/substrate/data_case/result/invalid_smiles.txt', 'a') as f:
    ...
    mutex = Lock()
    os.system('rm /data/cjm/substrate/data_case/raw/invalid_smiles.txt')
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_conformation_dict, open('/data/cjm/substrate/data_case/intermediate/smiles_to_conformation_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_conformation_dict))


def construct_data_list():
    data_df = pd.read_csv("/data/cjm/substrate/data_case/raw/substrate_rdkit.csv")
    smiles_to_conformation_dict = pkl.load(open('/data/cjm/substrate/data_case/intermediate/smiles_to_conformation_dict.pkl', 'rb'))
    data_list = []
    ...
    pkl.dump(data_list, open('/data/cjm/substrate/data_case/intermediate/data_list.pkl', 'wb'))


def convert_data_list_to_data_loader():
    def convert_data_list_to_dataset_(data_list):
        dictionary = Dictionary.load('/data/cjm/substrate/data_case/raw/token_list.txt')
    ...
    batch_size = 6  # 原来为8
    data_list = pkl.load(open('/data/cjm/substrate/data_case/intermediate/data_list.pkl', 'rb'))
...
class UniMolModel(nn.Module):
    def __init__(self):
        super().__init__()
        dictionary = Dictionary.load('/data/cjm/substrate/HumanTest1/raw/token_list.txt')
        dictionary.add_symbol("[MASK]", is_special=True)
...
class EsmUnimolClassifier(nn.Module):
    ...
    def forward(self, data_batch):
        ...
        # srs embedding + cls token
        srs_site_batch = data_batch['input']['SRS_site']
        for i in range(len(srs_site_batch)):
            srs = srs_site_batch[i].replace('(', '').replace(')', '')
            srs = srs.split(", ")
            srs_site_batch[i] = [int(p) for p in srs]
        # print("pocket_site_batch:", pocket_site_batch) #每个序列的pocket_site为多个位点
        max_srs_site = 0
        for srs_site in srs_site_batch:
            # 每个蛋白的srs为点数为srs_site中数值的综总和
            srs_num = 0
            for j in range(int(len(srs_site) / 2)):
                srs_num += srs_site[2 * j + 1] - srs_site[2 * j] + 1
            max_srs_site = max(max_srs_site, srs_num)

        srs_embedding = []
        for i in range(len(srs_site_batch)):  # 共有batch_size条序列，所以len(pocket_site_batch)为batch_size
            protein_embedding_i = protein_embedding[i]
            srs_embedding_i = []
            srs_num = int(len(srs_site_batch[i]) / 2)
            for j in range(srs_num):  # 每条序列的位点数为len(pocket_site_batch[i])
                srs_site_i = srs_site_batch[i]
                begin = int(srs_site_i[2 * j]) + 1
                end = int(srs_site_i[2 * j + 1]) + 2
                for n in range(begin, end):
                    srs_single = protein_embedding_i[n]  # 此时，srs_single的shape为(位点长度, 1024)
                    # 将srs_single的embedding合并,此时srs_single是一个矩阵，append可以合并矩阵的行
                    srs_embedding_i.append(srs_single)
            srs_embedding_i = th.stack(srs_embedding_i)

            if len(srs_embedding_i) < max_srs_site:  # 如果位点数较少，需要将其余的位点的embedding设置为0
                padding = th.zeros(max_srs_site - len(srs_embedding_i), 1280).cuda()
                srs_embedding_i = th.cat((srs_embedding_i, padding), dim=0)
            # print("pocket_embedding_i:", pocket_embedding_i.shape)
            srs_embedding.append(srs_embedding_i)
        srs_embedding = th.stack(srs_embedding) # 最终pocket_embedding的shape为(序列个数, 最大pocket_site数+1, 1280)
        # print("pocket_embedding:", pocket_embedding.shape)
        ...
        # 将蛋白和srs的embedding合并
        protein_sequence_srs_embedding = th.cat((protein_embedding, srs_embedding), dim=1)
        protein_sequence_pocket_embedding = th.cat((protein_sequence_srs_embedding, pocket_embedding), dim=1)
...
def evaluate(model, data_loader, csv_save):
    ...
    # print(label_predict.shape, label_true.shape)
    # print(label_predict)
    ...
    if csv_save == True:
        ...
        df.to_csv('label_predict_case_sub16.csv', index=False)
...
def train(trial_version):
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.000005, weight_decay=1e-6)
...
def test(trial_version):
    # data_loader_train, data_loader_validate, data_loader_test = convert_data_list_to_data_loader()
    data_loader_test = convert_data_list_to_data_loader()
    model = EsmUnimolClassifier()
    model.cuda()
    model.load_state_dict(th.load(f"/data/cjm/substrate/weight/{trial_version}.pt"))
...
if __name__ == "__main__":
    ...
    # print("train start!")
    # train(trial_version='new_sequence_srs_pocket_sub1')  # 表示训练集为全部训练集时的结果

    test(trial_version='new_sequence_srs_pocket_sub16') #权重名字
    print("test start!")
