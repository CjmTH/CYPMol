...
def calculate_molecule_3D_structure():
    def get_smiles_list_():
        data_df = pd.read_csv("../data_sub2/raw/train_1.csv")
    ...
    def calculate_molecule_3D_structure_(smiles_list):
        ...
        while True:
            ...
            if result != 0:
                ...
                mutex.acquire()
                with open('../data_sub2/result/invalid_smiles.txt', 'a') as f:
    ...
    mutex = Lock()
    os.system('rm ../data_sub2/result/invalid_smiles.txt')
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_conformation_dict, open('../data_sub2/intermediate/smiles_to_conformation_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_conformation_dict))


def construct_data_list():
    data_df = pd.read_csv("../data_sub2/raw/train_1.csv")
    smiles_to_conformation_dict = pkl.load(open('../data_sub2/intermediate/smiles_to_conformation_dict.pkl', 'rb'))
    data_list = []
    ...
    pkl.dump(data_list, open('../data_sub2/intermediate/data_list.pkl', 'wb'))


def convert_data_list_to_data_loader():
    def convert_data_list_to_dataset_(data_list):
        dictionary = Dictionary.load('../data_sub2/raw/token_list.txt')
    ...
    batch_size = 6  # 原来为8
    data_list = pkl.load(open('../data_sub2/intermediate/data_list.pkl', 'rb'))
...
def evaluate(model, data_loader, csv_save):
    ...
    if csv_save == True:
        ...
        df.to_csv('label_predict_sequence_srs_pocket_sub2.csv', index=False)
...
if __name__ == "__main__":
    ...
    # print("data_process start!") #新数据需要首先进行数据处理
    # calculate_molecule_3D_structure()
    # construct_data_list()

    print("train start!")
    train(trial_version='new_sequence_srs_pocket_sub2')  #权重的名字

    test(trial_version='new_sequence_srs_pocket_sub2')
    print("test start!")
