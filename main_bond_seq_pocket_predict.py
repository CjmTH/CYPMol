...
def calculate_molecule_3D_structure():
    def get_smiles_list_():
        data_df = pd.read_csv("/data/cjm/reaction_site/data_mutant_case/raw/BOM_rdkit_site_bond_label.csv")
    ...
    def calculate_molecule_3D_structure_(smiles_list):
        ...
        while True:
            ...
            if result != 0:
                ...
                mutex.acquire()
                with open('/data/cjm/reaction_site/data_mutant_case/result/invalid_smiles.txt', 'a') as f:
    ...
    mutex = Lock()
    os.system('rm /data/cjm/reaction_site/data_mutant_case/result/invalid_smiles.txt')
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_conformation_dict, open('/data/cjm/reaction_site/data_mutant_case/intermediate/smiles_to_conformation_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_conformation_dict))


def construct_data_list():
    data_df = pd.read_csv("/data/cjm/reaction_site/data_mutant_case/raw/BOM_rdkit_site_bond_label.csv")
    smiles_to_conformation_dict = pkl.load(open('/data/cjm/reaction_site/data_mutant_case/intermediate/smiles_to_conformation_dict.pkl', 'rb'))
    data_list = []
    ...
    pkl.dump(data_list, open('/data/cjm/reaction_site/data_mutant_case/intermediate/data_list.pkl', 'wb'))

def convert_data_list_to_data_loader():
    def convert_data_list_to_dataset_(data_list):
        dictionary = Dictionary.load('/data/cjm/reaction_site/data_mutant_case/raw/token_list.txt')
    ...
    batch_size = 4  # 原来为8
    data_list = pkl.load(open('/data/cjm/reaction_site/data_mutant_case/intermediate/data_list.pkl', 'rb'))
...
class UniMolModel(nn.Module):
    def __init__(self):
        super().__init__()
        dictionary = Dictionary.load('/data/cjm/reaction_site/data_valid_bome5_7/raw/token_list.txt')
        dictionary.add_symbol("[MASK]", is_special=True)
...
class EsmUnimolClassifier(nn.Module):
    def __init__(self):
        ...
        self.molecule_encoder.load_state_dict(th.load('../weight/mol_pre_all_h_220816.pt')['model'], strict=False)
    ...
    def forward(self, data_batch):
        ...
        # print("atom_index_batch:", atom_index_batch.shape)
        # for i in range(len(atom_index_batch)):
        #     print(i)
        #     print("atom_index:", atom_index_batch[i])
        #     print("atom_embedding:", molecule_embedding[i][atom_index_batch[i]+1])

        # 此时，每个键有两个原子，原子索引号分别为atom1_index和atom2_index，需要将每个原子的embedding取出来，并纵向拼接
        atom1_embedding = molecule_embedding[
            th.arange(molecule_embedding.size(0)), atom1_index_batch + 1]  # +1是因为分子的第一个token是cls
        atom2_embedding = molecule_embedding[
            th.arange(molecule_embedding.size(0)), atom2_index_batch + 1]  # +1是因为分子的第一个token是cls
        ...
        x = self.transformer_layer_cross_attention(bond_molecule_embedding, protein_sequence_pocket_embedding, None)
...
def evaluate(model, data_loader, csv_save):
    ...
    with th.no_grad():
        for data_batch in data_loader:
            ...
            label_predict_batch = model(data_batch).to(devices[0])
            label_true_batch = data_batch['target']['label'].to(th.long).to(devices[0])
    ...
    if csv_save == True:
        ...
        df.to_csv('bond_label_predict_bond_bs8_seq_pocket_ft_valid_mutant.csv', index=False)
...
def train(devices, trial_version):
    ...
    model.cuda()
    model = nn.DataParallel(module=model, device_ids=devices).to(devices[0])
...
def test(devices, trial_version):
    # data_loader_train, data_loader_validate, data_loader_test = convert_data_list_to_data_loader()
...
if __name__ == "__main__":
    ...
    # train(devices, trial_version='bond_bs8_seq_pocket_ft')  # 表示训练集为全部训练集时的结果

    print("test start!")
    devices = d2l.torch.try_all_gpus()[0:2]
    test(devices, trial_version='bond_bs8_seq_pocket_ft')
