...
def calculate_molecule_3D_structure():
    def get_smiles_list_():
        data_df = pd.read_csv("../SRS_bond_data/raw/bond_label.csv")
    ...
    mutex = Lock()
    os.system('rm ../SRS_bond_data/result/invalid_smiles.txt')
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_conformation_dict, open('../SRS_bond_data/intermediate/smiles_to_conformation_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_conformation_dict))


def construct_data_list():
    data_df = pd.read_csv("../SRS_bond_data/raw/bond_label.csv")
    smiles_to_conformation_dict = pkl.load(open('../SRS_bond_data/intermediate/smiles_to_conformation_dict.pkl', 'rb'))
    data_list = []
    ...
    pkl.dump(data_list, open('../SRS_bond_data/intermediate/data_list.pkl', 'wb'))

def convert_data_list_to_data_loader():
    def convert_data_list_to_dataset_(data_list):
        dictionary = Dictionary.load('../SRS_bond_data/raw/token_list.txt')
    ...
    batch_size = 4  # 原来为8
    data_list = pkl.load(open('../SRS_bond_data/intermediate/data_list.pkl', 'rb'))
...
def evaluate(model, data_loader, csv_save):
    ...
    if csv_save == True:
        ...
        df.to_csv('bond_label_predict_bond_bs8_seq_pocket_ft.csv', index=False)
